...
- [ ] T001 [P] Update `.gitignore`: add `session-cookies.json` as a belt-and-braces entry (the
...
- [ ] T002 Add `SESSION_COOKIE_FILENAME = "session-cookies.json"` and
...
- [ ] T003 [P] Write import tests in `tests/test_session.py`: state file absent -> zero calls to
...
- [ ] T004 [P] Write export tests in `tests/test_session.py`: a fake context's `cookies()`
  ...
  export call
- [ ] T005 [P] Write `add_cookies`-failure and no-leak tests in `tests/test_session.py`: a fake
...
- [ ] T006 Implement `_import_session_cookies(context, profile_dir: Path) -> None` in
  ...
  re-raises
- [ ] T007 Implement `_export_session_cookies(context, profile_dir: Path) -> None` in
...
- [ ] T008 Run `python -m pytest -q tests/test_session.py -k cookie` and make T003-T005 green
...
- [ ] T009 [P] [US1] Write a sequential-launch test in `tests/test_session.py`: using two fake
...
- [ ] T010 [US1] Wire `_import_session_cookies(self.context, self.config.profile_dir)` into
...
- [ ] T011 [US1] Wire `_export_session_cookies(self.context, self.config.profile_dir)` into
  `Session.__exit__`, launched-profile branch only, immediately before `self.context.close()`
- [ ] T012 [P] [US1] Add the opt-in real-browser test to `tests/test_gates_browser.py`, under the
...
- [ ] T014 [P] [US2] Write a launch-kwargs test in `tests/test_session.py`: monkeypatch the
...
- [ ] T015 [P] [US2] Write a matching launch-kwargs test in `tests/test_check_env.py`:
...
- [ ] T016 [US2] Add `chromium_sandbox=True` to the `launch_persistent_context(...)` call in
  `headless/session.py`'s `Session.__enter__`
- [ ] T017 [US2] Add `chromium_sandbox=True` to the `p.chromium.launch(channel="chrome",
...
- [ ] T019 [P] [US3] Write a CDP-attach exclusion test in `tests/test_session.py`: construct a
  ...
  SC-006)
- [ ] T020 [P] [US3] Write a consolidated no-leak test in `tests/test_session.py`: run every
...
- [ ] T021 [US3] Add a dedicated regression test in `tests/test_session.py`, if not already
...
- [ ] T022 [US3] Run quickstart Scenario 4 by hand: after a real Scenario 1 run, inspect
  ...
  (`pytest -k cdp_cookie`) and confirm it passes
...
- [ ] T023 [P] Update `CLAUDE.md`: one sentence in the Browser section and one in the Secrets
...
- [ ] T024 Regenerate `.specify/memory/constitution.md` to **1.2.1** (PATCH: wording only - this
...
- [ ] T025 [P] Add two new entries to `PATTERNS.md`: "Session cookie persistence" (summarizing
...
- [ ] T026 [P] Update `README.md`: one sentence in Setup step 7 (a seeded login now persists
...
- [ ] T027 Add the v0.0.3 Changelog row to `Project_Structure.md` listing every file touched
...
- [ ] T028 [P] Update `MEMORY.md`: add the 2026-08-25 UAT result rows (check_env 5/5 PASS, plain
...
- [ ] T029 Run the commit gate: `python -m pytest -q && python scripts/verify_structure.py &&
