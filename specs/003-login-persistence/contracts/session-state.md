...
| The write itself fails (directory unwritable, disk full, permission error, or any other `OSError` at any step of the temp-file-then-replace sequence) | Caught; the temporary file is removed best-effort (`unlink(missing_ok=True)`, itself wrapped so a second failure here can never raise) before the note prints. | Exactly one line: `note: session cookies not saved (<ExceptionClassName>)`. `context.close()` still runs immediately after; the run's exit code and every other output are unaffected. No `.tmp` file is left behind at any point past this failure. |
| An import from an existing state file failed this run (Opus verifier FIX-FIRST 1, 2026-08-25) | Export is skipped entirely; `context.cookies()` is never called. | Nothing printed beyond the one note the import already produced. The file is left exactly as it was before this run - a transient import failure can never overwrite a still-good file with an empty one. |
