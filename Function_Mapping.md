...
| `scripts/activity_scan.py` | Google Maps list view + place pages | result cards (name, coordinates, rating, review count, category, address, today's hours) and, for the top `--details` ranked venues, that venue's own place page (weekly hours, website, phone, address) | nothing on the site - local reports only (`reports/activity/activity-scan-<date>.md`/`.json`) | none | `n/a (read-only errand)`: `--apply` is refused |
| `scripts/product_scan.py` | Amazon search results (primary) + Home Depot search results (opt-in) + an Amazon or Walmart product page (`--reference-url`, reference only) | search result cards/pods (title, price, rating, review count, sponsored flag) and, for a `--reference-url`, that product's own page (title, price, rating, review count) | nothing on any site - local reports only (`reports/product/product-scan-<slug>-<date>.md`/`.json`, the query's own slug before the date, fix batch A5/B9, 2026-09-11) | none | `n/a (read-only errand)`: `--apply` is refused |
